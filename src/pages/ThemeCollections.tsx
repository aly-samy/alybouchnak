...
const ThemeCollections = () => {
    ...
    const categories = [
        { id: 'Routine & Utility', title: 'The Routine & Utility Collection', subtitle: 'The Problem Solvers' },
        { id: 'Mood & Energy', title: 'The Mood & Energy Collection', subtitle: 'The Vibe Managers' },
        { id: 'Adventure & Event', title: 'The Adventure & Event Collection', subtitle: 'The Memory Makers' },
        { id: 'Signature Collections', title: 'Signature Collections', subtitle: 'Curated Picks & Essentials' }
    ] as const;
        ...
        <div className="relative min-h-screen bg-[#C8F0F7]">
            ...
            <div className="pb-24 space-y-24">
                {categories.map((category) => {
                    const categoryCollections = collections.filter(c => c.category === category.id);
                    if (categoryCollections.length === 0) return null;

                    return (
                        <section key={category.id} className="w-full px-4 sm:px-6 lg:px-8 xl:px-12 max-w-7xl mx-auto">
                            <div className="mb-10 lg:mb-12">
                                <h2 className="font-['Fredoka_One'] text-3xl sm:text-4xl lg:text-5xl text-[#101010] mb-3">
                                    {category.title}
                                </h2>
                                <p className="text-xl text-[#2A2A2A]/80 font-medium">"{category.subtitle}"</p>
                            </div>
